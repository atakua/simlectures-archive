...
\node[rectangle split, rectangle split parts=4, draw, right=2.5cm of templates-raw, minimum width=1.8cm] (templates) {Шаблон 1\nodepart{two} Шаблон 2\nodepart{three} Шаблон 3\nodepart{four} Шаблон 4};

\draw[->] (decode) -- (templates-raw.text west) node[midway, above]{\tiny Выбор шаблонов};
\draw[->] (templates-raw) -- (templates) node[midway, above]{\tiny Подстановка аргументов};
\coordinate[above=1cm of templates] (junction);
\draw[->] (decode) |- (junction) -- (templates);
